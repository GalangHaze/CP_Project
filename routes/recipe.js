...
//     const insertQuery = 'INSERT INTO userrecipe (user_id, img_url) VALUES (?, ?)';
//     const values = [user_id, img_url];
...
//       res.send('Userrecipe foto added successfully');
//     });
//   });
...
//     const updateQuery = 'UPDATE userrecipe SET img_url = ? WHERE user_id = ?';
//     const values = [img_url, user_id];
  
//     connection.query(updateQuery, values, (err, results) => {
//       if (err) {
//         console.error('Error updating userrecipe foto:', err);
//         res.status(500).json({ error: 'Internal server error' });
//         return;
//       }
  
//       res.send('Userrecipe foto updated successfully');
//     });
//   });

router.post("/userrecipe", multer.single('img_url'), imgUpload.uploadToGcs, (req, res) => {
  const user_id = uuidv4();
  const nama_profil = req.body.nama_profil
  var imageUrl = ''
  ...
  const query = "INSERT INTO userrecipe (user_id, nama_profile, img_url) values (?, ?, ?,)"

  connection.query(query, [user_id, nama_profil, imageUrl], (err, rows, fields) => {
      if (err) {
...
router.put("/userrecipe/:id", multer.single('img_url'), imgUpload.uploadToGcs, (req, res) => {
  ...
  const query = "UPDATE userrecipe SET nama_profil = ?, img_url = ? WHERE id = ?"
  
  connection.query(query, [nama_profil, imageUrl, user_id], (err, rows, fields) => {
      if (err) {
          res.status(500).send({message: err.sqlMessage})
      } else {
...
router.get('/pantry/:userId', (req, res) => {
    ...
    const query = `
      SELECT ingredient.ing_id, ingredient.ing_name
      FROM pantry
